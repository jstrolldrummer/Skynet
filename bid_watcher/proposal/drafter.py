...
def build_placeholder_map(email: IncomingEmail, c: BidClassification) -> dict[str, str]:
    return {
...
def render_proposal(template_path: Path, email: IncomingEmail, c: BidClassification) -> bytes:
    if not template_path.exists():
        log.warning("proposal.template_missing", path=str(template_path))
        return _render_fallback(email, c)
    doc = Document(str(template_path))
    _fill_doc(doc, build_placeholder_map(email, c))
    buf = io.BytesIO()
    doc.save(buf)
    return buf.getvalue()


def _render_fallback(email: IncomingEmail, c: BidClassification) -> bytes:
    """Minimal proposal when no template is configured yet."""
    doc = Document()
    doc.add_heading("PROPOSAL — DRAFT", level=0)
    mapping = build_placeholder_map(email, c)
    for label, key in [
